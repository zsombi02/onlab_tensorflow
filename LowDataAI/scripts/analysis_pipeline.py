...
def run_classwise_comparison():
    ...
    plot_classwise_comparison(models=modelgroup_quartered_augmented, metric=Metric.F1.value, title="quartered_dataset_augmented", save_path=ANALYSIS_CLASSWISE_DIR)


def run_overall_comparison():

    plot_accuracy_and_loss_from_metrics(save_path=ANALYSIS_OVERALL_DIR, title="quartered_augmented", model_group=modelgroup_quartered_augmented)
